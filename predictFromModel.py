...
class prediction:
    ...
    def predictionFromModel(self):
        ...
        try:
            self.pred_data_val.deletePredictionFile() #deleted the existing prediction file from last run!
            ...
            cols_to_drop  = preprocessor.get_columns_with_zero_std_devitaion(std)
            if(is_null_present):
                data= data.preprocessor.impute_missing_values(data)
            ...
            for i in clusters:
                ...
                wafer_names = list(cluster_data['Wafer'],axis=1) #drops the first column for cluster prediction
                ...
                for i in clusters:
                    ...
                    result.to_csv("Prediction_Output_File/Predictions.csv",header=True,mode='a+') #appends result to prediciton file
